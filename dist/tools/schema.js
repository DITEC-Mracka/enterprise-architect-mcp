...
export function configureSchemaTools(server, db) {
    server.tool("ea_get_schema", "List the model's database tables, or show columns and indexes for a specific table. Use this to discover what data the model holds beyond what the typed ea_* tools return. See ea_get_model_info for the export's identity.", {
        tableName: z
            .string()
            .optional()
    ...
    server.tool("ea_get_model_info", "Report which .qea export file the server has open — its file name (the citable identity), size, and modification time. The full resolved path is also available as local detail.", {}, async () => {
        try {

#!/usr/bin/env node
/**
 * Eval runner: starts the MCP server, executes eval tasks, scores assertions.
 * Usage: npx tsx eval/runner.ts <path-to-qea> [tasks-json]
 */
...
async function main() {
  ...
  if (!qeaPath) {
    console.error("Usage: npx tsx eval/runner.ts <path-to-qea> [tasks-json]");
    process.exit(1);
  }

  const tasksPath = process.argv[3] || resolve(__dirname, "tasks.json");
  const tasksJson = await readFile(tasksPath, "utf-8");
  const tasks: EvalTask[] = JSON.parse(tasksJson);

  console.log(`Starting MCP server with ${qeaPath}...`);
  const serverPath = resolve(__dirname, "..", "dist", "index.js");

  const transport = new StdioClientTransport({
    command: "node",
    args: [serverPath, resolve(qeaPath)],
  });

...
export function configureScenarioTools(server: McpServer, db: Database): void {
  server.tool(
    "ea_get_scenarios",
    "Get use case scenario flows for an element. Returns parsed step-by-step flows with all step attributes (trigger, uses, result, state, link) and scenario-level notes. Steps are numbered within each scenario. Scenarios ordered by type: Basic Path first, then Alternate, then Exception.",
    {

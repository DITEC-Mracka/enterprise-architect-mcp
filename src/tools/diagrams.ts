...
export function configureDiagramTools(server: McpServer, db: Database): void {
  server.tool(
    "ea_get_diagram_elements",
    "Get all elements and connectors placed on a specific diagram. Connectors include feature-link resolution showing which attribute or operation each end attaches to. The connector list is the union of explicit t_diagramlinks rows and implied connectors (both ends on the diagram).",
    {
  ...
  server.tool(
    "ea_list_diagrams",
    "List diagrams in the model, optionally filtered by package and/or name substring. Returns diagram ID, name, type, package path, and ea_guid.",
    {

...
describe("ea_search", () => {
  it("finds elements by name", async () => {
    ...
    expect(data.results.some((e: any) => e.Name === "Správa väzňov")).toBe(true);
  ...
  it("includes PackageName in results", async () => {
    ...
    const data = res.json();
    const uc = data.results.find((e: any) => e.Name === "Správa väzňov");
  ...
  it("carries notePreviewTruncated flag on long notes", async () => {
    const res = await callTool("ea_search", { query: "výživné" });
    const data = res.json();
    const req = data.results.find((e: any) => e.Name === "Požiadavka na výživné");
...
describe("ea_get_element", () => {
  ...
  it("returns diagrams the element appears on (R4)", async () => {
    ...
    expect(data.diagrams[0].name).toBe("UC Správa väzňov");
  ...
  it("returns constraints on element (R10)", async () => {
    ...
    expect(process.notes).toContain("LOG_FEO_083");
  });
...
describe("ea_get_connectors", () => {
  ...
  it("filters outgoing connectors", async () => {
    ...
    expect(data.connectors).toHaveLength(1); // Association to Väzeň
  });
  ...
  it("resolves feature link to attribute via StyleEx LFSP token", async () => {
    ...
    expect(real.sourceFeature.owningElementName).toBe("Väzeň");
  ...
  it("returns SourceRole and DestRole", async () => {
    ...
    expect(assoc.destRole).toBe("Väzeň");
  });
});
...
describe("ea_resolve", () => {
  it("resolves braced GUID to element", async () => {
    ...
    expect(el.name).toBe("Správa väzňov");
  ...
  it("resolves GUID with kind=diagram filter", async () => {
    ...
    expect(data.candidates[0].name).toBe("UC Správa väzňov");
  });

  it("resolves unique name to single candidate", async () => {
    const res = await callTool("ea_resolve", { reference: "Väzeň" });
    const data = res.json();
    expect(data.totalMatched).toBe(1);
    expect(data.candidates[0].name).toBe("Väzeň");
  });
  ...
  it("resolves duplicate diagram name to multiple candidates", async () => {
    const res = await callTool("ea_resolve", { reference: "UC Správa väzňov", kind: "diagram" });
    const data = res.json();
    expect(data.totalMatched).toBe(2);
  });
...
describe("ea_list_diagrams", () => {
  ...
  it("filters by package", async () => {
    ...
    const data = res.json();
    expect(data.results.length).toBe(1);
    expect(data.results[0].name).toBe("UC Správa väzňov");
  });
